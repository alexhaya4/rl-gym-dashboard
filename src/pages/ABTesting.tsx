    ...
    <div className="space-y-6">
        ...
        <Card>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold">
              Statistics — <span className="font-mono text-accent">{statistics.id}</span>
            </h3>
            <Button variant="ghost" size="sm" onClick={() => setStatistics(null)}>
              Close
            </Button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-xs">
            <div>
              ...
              <span className="font-mono font-bold">
                {statistics.data.model_a_mean_reward?.toFixed(2) ?? '—'}
              </span>
            </div>
            <div>
              <span className="dark:text-dark-text-secondary text-light-text-secondary">
              ...
              <span className="font-mono font-bold">
                {statistics.data.model_b_mean_reward?.toFixed(2) ?? '—'}
              </span>
            </div>
            <div>
              <span className="dark:text-dark-text-secondary text-light-text-secondary">
              ...
              <span className="font-mono">{statistics.data.p_value?.toFixed(4) ?? '—'}</span>
            </div>
            <div>
              <span className="dark:text-dark-text-secondary text-light-text-secondary">
                Model A Episodes:
              </span>{' '}
              <span className="font-mono">{statistics.data.model_a_n_episodes}</span>
            </div>
            <div>
              <span className="dark:text-dark-text-secondary text-light-text-secondary">
                Model B Episodes:
              </span>{' '}
              <span className="font-mono">{statistics.data.model_b_n_episodes}</span>
            </div>
            <div>
              <span className="dark:text-dark-text-secondary text-light-text-secondary">
                Significant:
              ...
              <span
                className={`font-mono font-bold ${statistics.data.is_significant ? 'text-emerald-400' : 'text-amber-400'}`}
              >
            ...
            {statistics.data.winner && (
              <div>
                ...
                <span className="font-mono font-bold">{statistics.data.winner}</span>
              </div>
            )}
            {statistics.data.effect_size != null && (
              <div>
                ...
                <span className="font-mono">{statistics.data.effect_size.toFixed(4)}</span>
              </div>
            )}
            <div>
              <span className="dark:text-dark-text-secondary text-light-text-secondary">
              ...
              <span className="font-mono">
                {(statistics.data.confidence_level * 100).toFixed(1)}%
              </span>
            </div>
          </div>
        </Card>
      ...
      {results && (
        <Card padding="none">
          <div className="flex items-center justify-between p-5 border-b dark:border-dark-border border-light-border">
            <h3 className="text-sm font-semibold">
              Results — <span className="font-mono text-accent">{results.id}</span>
            </h3>
                  ...
                  {results.data.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-3">
                        <Badge variant={r.model_variant === 'A' ? 'info' : 'accent'}>
                          {r.model_variant}
                        </Badge>
                      </td>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs dark:text-dark-text-secondary text-light-text-secondary">
                      {test.description ||
                        `${test.n_eval_episodes_per_model} episodes/model — ${test.statistical_test}`}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-3 rounded-[var(--radius-card)] dark:bg-dark-bg bg-light-bg">
                      ...
                      <p className="text-lg font-bold font-mono">
                        {test.statistics.model_a_mean_reward?.toFixed(2) ?? '—'}
                      </p>
                      <p className="text-xs dark:text-dark-text-secondary text-light-text-secondary">
                    ...
                    <div className="p-3 rounded-[var(--radius-card)] dark:bg-dark-bg bg-light-bg">
                      ...
                      <p className="text-lg font-bold font-mono">
                        {test.statistics.model_b_mean_reward?.toFixed(2) ?? '—'}
                      </p>
                      <p className="text-xs dark:text-dark-text-secondary text-light-text-secondary">
                  ...
                  <div className="flex items-center gap-3 p-3 rounded-[var(--radius-card)] border dark:border-dark-border border-light-border">
                    ...
                    <div>
                      ...
                      <p className="text-xs dark:text-dark-text-secondary text-light-text-secondary">
                        p-value: {test.statistics.p_value?.toFixed(4) ?? '—'} | effect size:{' '}
                        {test.statistics.effect_size?.toFixed(4) ?? '—'}
                      </p>
                    </div>
                  </div>
                </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Create A/B Test">
        ...
        >
          ...
          <Input
            label="Model Version A ID"
            type="number"
            value={form.model_version_a_id}
            onChange={(e) => setForm({ ...form, model_version_a_id: Number(e.target.value) })}
            required
          />
          <Input
            label="Model Version B ID"
            type="number"
            value={form.model_version_b_id}
            onChange={(e) => setForm({ ...form, model_version_b_id: Number(e.target.value) })}
            required
          />
          <Input
            ...
            onChange={(e) => setForm({ ...form, traffic_split_a: parseFloat(e.target.value) })}
            required
          />
          <Input
            label="N Eval Episodes Per Model"
            type="number"
            value={form.n_eval_episodes_per_model}
            onChange={(e) =>
              setForm({ ...form, n_eval_episodes_per_model: Number(e.target.value) })
            }
            required
          />
          <Input
            ...
            type="number"
            step="0.01"
            value={form.significance_level}
            onChange={(e) => setForm({ ...form, significance_level: parseFloat(e.target.value) })}
            required
          />
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium dark:text-dark-text-secondary text-light-text-secondary">
