        ...
        <Card>
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs dark:text-dark-text-secondary text-light-text-secondary mt-0.5">
                {lastResult.total_combinations} combinations — completed {new Date(lastResult.completed_at).toLocaleString()}
              </p>
            </div>
          </div>
          ...
          {lastResult.results.length > 0 ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div className="h-64">
                <p className="text-xs font-medium dark:text-dark-text-secondary text-light-text-secondary mb-2">Mean Reward</p>
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={chartData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="var(--color-dark-border)" />
                    <XAxis dataKey="label" stroke="var(--color-dark-text-secondary)" fontSize={11} />
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b dark:border-dark-border border-light-border">
                      <th className="text-left py-2 text-xs font-medium dark:text-dark-text-secondary text-light-text-secondary">Environment</th>
                      <th className="text-left py-2 text-xs font-medium dark:text-dark-text-secondary text-light-text-secondary">Algorithm</th>
                      <th className="text-right py-2 text-xs font-medium dark:text-dark-text-secondary text-light-text-secondary">Mean Reward</th>
                      <th className="text-right py-2 text-xs font-medium dark:text-dark-text-secondary text-light-text-secondary">Std Reward</th>
                      <th className="text-right py-2 text-xs font-medium dark:text-dark-text-secondary text-light-text-secondary">Time (s)</th>
                    </tr>
                  </thead>
                  <tbody>
                    {lastResult.results.map((r, i) => (
                      <tr key={i} className="border-b last:border-b-0 dark:border-dark-border border-light-border">
                        ...
                        <td className="py-2 text-right font-mono text-xs">{r.mean_reward.toFixed(2)}</td>
                        <td className="py-2 text-right font-mono text-xs">{r.std_reward.toFixed(2)}</td>
                        <td className="py-2 text-right font-mono text-xs">{r.training_time_seconds.toFixed(1)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Run Benchmark">
        ...
        >
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium dark:text-dark-text-secondary text-light-text-secondary">Environments</label>
            ...
            <div className="flex flex-wrap gap-1">
              {runForm.environments.map((env) => (
                <span key={env} className="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded dark:bg-dark-hover bg-light-hover font-mono">
                  {env}
                  <button type="button" onClick={() => setRunForm({ ...runForm, environments: runForm.environments.filter((e) => e !== env) })} className="text-red-400 hover:text-red-300 cursor-pointer">&times;</button>
                </span>
              ))}
            </div>
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-sm font-medium dark:text-dark-text-secondary text-light-text-secondary">Algorithms</label>
            <div className="flex gap-2">
              <select
                value={algoInput}
            ...
            <div className="flex flex-wrap gap-1">
              {runForm.algorithms.map((alg) => (
                <span key={alg} className="inline-flex items-center gap-1 px-2 py-0.5 text-xs rounded dark:bg-dark-hover bg-light-hover font-mono">
                  {alg}
                  <button type="button" onClick={() => setRunForm({ ...runForm, algorithms: runForm.algorithms.filter((a) => a !== alg) })} className="text-red-400 hover:text-red-300 cursor-pointer">&times;</button>
                </span>
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button type="submit" loading={runMutation.isPending}>Run</Button>
          </div>
        </form>
      </Modal>
    </div>
