...
export default function Training() {
  ...
  const startMutation = useMutation({
    ...
    onSuccess: (response) => {
      ...
      setModalOpen(false);
      setSubmitError(null);
      setForm({ environment_id: 'CartPole-v1', algorithm: 'PPO', total_timesteps: 10000, hyperparameters: {}, n_envs: 1, experiment_name: '' });
      setLearningRate('');
  ...
  return (
    <div className="space-y-6">
      ...
      {selectedSession != null && (
        <Card>
          <div className="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="sm" onClick={() => { setSelectedSession(null); setLiveMetrics([]); wsRef.current?.close(); }}>
              Close
            </Button>
          </div>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={liveMetrics}>
                ...
                />
                <Line type="monotone" dataKey="episode_reward" name="Reward" stroke="#d97757" strokeWidth={2} dot={false} />
                <Line type="monotone" dataKey="loss" name="Loss" stroke="#60a5fa" strokeWidth={1.5} dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      {jobInfo && (
        <Card>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold">Job Status — <span className="font-mono text-accent">{jobInfo.id}</span></h3>
            <Button variant="ghost" size="sm" onClick={() => setJobInfo(null)}>Close</Button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Job ID:</span> <span className="font-mono">{jobInfo.data.id.slice(0, 12)}</span></div>
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Status:</span> <span className="font-mono">{jobInfo.data.status}</span></div>
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Enqueued:</span> <span className="font-mono">{new Date(jobInfo.data.enqueued_at).toLocaleTimeString()}</span></div>
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Started:</span> <span className="font-mono">{jobInfo.data.started_at ? new Date(jobInfo.data.started_at).toLocaleTimeString() : '—'}</span></div>
          </div>
          {jobInfo.data.error && (
      ...
      {resultInfo && (
        <Card>
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-semibold">Result — <span className="font-mono text-accent">{resultInfo.id}</span></h3>
            <Button variant="ghost" size="sm" onClick={() => setResultInfo(null)}>Close</Button>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-xs">
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Mean Reward:</span> <span className="font-mono font-bold">{resultInfo.data.mean_reward?.toFixed(2) ?? '—'}</span></div>
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Std Reward:</span> <span className="font-mono">{resultInfo.data.std_reward?.toFixed(2) ?? '—'}</span></div>
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Timesteps:</span> <span className="font-mono">{resultInfo.data.total_timesteps.toLocaleString()}</span></div>
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Elapsed:</span> <span className="font-mono">{resultInfo.data.elapsed_time != null ? `${resultInfo.data.elapsed_time.toFixed(1)}s` : '—'}</span></div>
            <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Algorithm:</span> <span className="font-mono">{resultInfo.data.algorithm}</span></div>
            {resultInfo.data.model_path && (
              <div><span className="dark:text-dark-text-secondary text-light-text-secondary">Model:</span> <span className="font-mono">{resultInfo.data.model_path}</span></div>
            )}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b dark:border-dark-border border-light-border">
                  ...
                  <th className="text-left px-5 py-3 text-xs font-medium dark:text-dark-text-secondary text-light-text-secondary uppercase tracking-wider">Timesteps</th>
              ...
              <tbody>
                {sessions.map((session) => (
                  <tr key={session.experiment_id} className="border-b last:border-b-0 dark:border-dark-border border-light-border dark:hover:bg-dark-hover/50 hover:bg-light-hover/50">
                    ...
                    <td className="px-5 py-3 font-mono text-xs">{session.total_timesteps.toLocaleString()}</td>
                    <td className="px-5 py-3 font-mono text-xs">
                      {session.mean_reward != null ? session.mean_reward.toFixed(2) : '—'}
                      {session.elapsed_time != null && (
                        <span className="dark:text-dark-text-secondary text-light-text-secondary ml-2">({session.elapsed_time.toFixed(0)}s)</span>
                      )}
                    </td>
                    <td className="px-5 py-3 text-right">
                      <div className="flex items-center justify-end gap-1">
                        {(session.status === 'running' || session.status === 'queued') && (
                          <>
                            <Button variant="ghost" size="sm" onClick={() => connectWs(session.experiment_id)}>
                              <RefreshCw size={13} />
                              Live
                            </Button>
                            <Button variant="ghost" size="sm" onClick={() => fetchJob(session.experiment_id)}>
                              <BarChart3 size={13} />
                              Job
                            </Button>
                        ...
                        {session.status === 'completed' && (
                          <Button variant="ghost" size="sm" onClick={() => fetchResult(session.experiment_id)}>
                            <FileText size={13} />
                            Result
                          </Button>
                        )}
      ...
      <Modal open={modalOpen} onClose={() => { setModalOpen(false); setSubmitError(null); }} title="Start Training">
        <form
          onSubmit={(e) => {
            e.preventDefault();
            setSubmitError(null);
            ...
            startMutation.mutate({ ...form, hyperparameters: hp, experiment_name: form.experiment_name || undefined });
          }}
          className="space-y-4"
        >
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="secondary" type="button" onClick={() => { setModalOpen(false); setSubmitError(null); }}>
              Cancel
            </Button>
