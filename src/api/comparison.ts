...
export const comparisonApi = {
  compare: async (experimentIds: number[]): Promise<Record<string, unknown>> => {
    const res = await apiClient.post<Record<string, unknown>>('/comparison/', { experiment_ids: experimentIds });
    return res.data;
  },
  diff: async (expIdA: number, expIdB: number): Promise<Record<string, unknown>> => {
    const res = await apiClient.get<Record<string, unknown>>(`/comparison/diff/${expIdA}/${expIdB}`);
    return res.data;
  },
  ...
  setTags: async (experimentId: number, tags: string[]): Promise<Record<string, unknown>> => {
    const res = await apiClient.patch<Record<string, unknown>>(`/comparison/experiments/${experimentId}/tags`, tags);
    return res.data;
  },
